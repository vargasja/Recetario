...
L<Plack> ofrece muchos adaptadores de servidor y protocolo para que puedas escojer 
como C<FCGI>, C<SCGI> y C<mod_perl>.
Asegurate de ejecutar C<plackup> en tu directorio de inicio de aplicaciones, si no
...
colocarlo en tu script de aplicación porque él automaticamente se comportará como uno si detecta la 
...
Pero tambien puedes usar un middleware en tu aplicación. 
...
inverso que pasa informacion adicional con el encabezado C<X-*>.
...
El evento C<finish> sera emitido inmediatamente despues de que la conexión WebSocket ha
sido cerrada. 

=head2 Probando WebSockets

Mientras que la corriente de mensajes en las conexiónes WebSocket puede ser bastante dinámica, es
...
el envio de datos de clientes a los servidores, sin embargo la ventaja es bajos
...
Extraer informacion de sitios web nunca ha sido tan divertido antes. 
...
El drenaje de un callback pasado a C<write> sera invocado cuando todo el
...
Hacks divertidas que tal vez no usas con frequencia, pero puede ser que sean util un dia. 
...
Funciona porque todas las rutas sin un patron por defecto a C</> y automaticamente
se activa aunque no haya codigo real que se ejecuta por el enrutador. 
